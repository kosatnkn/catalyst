...
func newDBAdapter(t *testing.T) persistence.DatabaseAdapter {
	cfg := postgres.Config{
		Host:     "localhost",
		Port:     5432,
		Database: "sample",
	...
	a, err := postgres.NewDatabaseAdapterPostgres(cfg)
	if err != nil {
...
func TestInsert(t *testing.T) {
	...
	need := 1
	got := int(r[0][persistence.DatabaseAffectedRows].(int64))
	if got != need {
		t.Errorf("Affected rows: need `%d`, got `%d`", need, got)
	}

	need = 1
	got = int(r[0][persistence.DatabaseLastInsertID].(int64))
	if got != need {
...
func TestUpdate(t *testing.T) {
	...
	need = nil
	got = r[0][persistence.DatabaseLastInsertID]
	if got != need {
		t.Errorf("Last insert id: need `%d`, got `%d`", need, got)
	}
